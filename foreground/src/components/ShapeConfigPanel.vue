  ...
  <div class="shape-config-panel" @mousedown.stop @mousemove.stop @mouseup.stop @click.stop @wheel.stop.prevent="handleWheel">
    ...
    <div class="shape-list">
      <div
        ...
        :key="shape.name"
        class="shape-item"
        :class="{ active: toolStore.shapeConfig.type === shape.name }"
        @click.stop="setShapeType(shape.name)"
      >
        <SvgIcon :name="shape.icon" size="22px" :color="toolStore.shapeConfig.type === shape.name ? '#fff' : '#298fff'" />
      </div>
    </div>

    <!-- 当前图形信息 -->
    <div class="shape-info-card">
      <div class="shape-info-icon">
        <SvgIcon :name="currentShape.icon" size="24px" />
      </div>

      <div class="shape-info-text">
        <div class="shape-title">{{ currentShape.label }}</div>
        <div class="shape-subtitle">流程图</div>
      </div>
    ...
    <div class="config-section">
      ...
      <input
        ...
        min="1"
        max="20"
        :value="toolStore.shapeConfig.strokeWidth"
    ...
    <!-- 默认颜色 -->
    <div class="config-section">
      <div class="config-title color-title">默认颜色</div>
